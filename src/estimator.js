const covid19ImpactEstimator = (data) => {
  ...
  const casesByRequestedTime = Math.ceil(
    impactInfectionsByRequestedTime * 0.15
  );
  const totalBeds = Math.ceil(totalHospitalBeds * 0.35);
  const impactHospitalBedsByRequestedTime = Math.ceil(
    totalBeds - casesByRequestedTime
  );
  ...
  const impactDollarsInFlight = Math.floor(
